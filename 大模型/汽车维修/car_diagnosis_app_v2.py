...
def main():
    ...
    # 显示诊断结果
        ...
        # **显示历史诊断记录**
        with st.expander("📝 诊断历史", expanded=True):
            for entry in st.session_state.diagnosis_history:
                if entry["stage"] == "question_answers":
                    ...
                    st.markdown("### 诊断结果")
                    st.markdown(f"分析: {entry['diagnosis_result']['analysis']}")
                    # 可添加其他历史字段（如维修建议、门店推荐等）
